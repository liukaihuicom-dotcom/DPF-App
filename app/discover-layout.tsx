...
export default function DiscoverLayoutScreen() {
  ...
  const modeLabels: Record<DiscoverLayoutViewMode, string> = {
    large: locale === 'en-US' ? 'Large' : '大卡',
    list: locale === 'en-US' ? 'List' : '列表',
    medium: locale === 'en-US' ? 'Medium' : '中卡',
  };
  const title = locale === 'en-US' ? 'Layout settings' : '布局设置';
  const close = () => {
    if (router.canGoBack()) {
      router.back();
      return;
    ...
    <Screen
      ...
      stickyFooter={
        <View style={styles.footerActions}>
          <ActionButton label={locale === 'en-US' ? 'Cancel' : '取消'} onPress={close} style={StyleSheet.flatten([styles.footerButton, { backgroundColor: palette.panelSoft }])} tone="neutral" />
          <ActionButton label={locale === 'en-US' ? 'Save' : '保存'} onPress={save} style={styles.footerButton} tone="neutral" />
        </View>
      }
      title={title}>
      <View style={StyleSheet.flatten([styles.introCard, { backgroundColor: palette.panel, borderColor: palette.lineSoft }])}>
        ...
        <View style={styles.flex}>
          ...
          <AppText numberOfLines={2} tone="muted" variant="caption">
            {locale === 'en-US' ? 'Drag to reorder modules, then choose how each one appears on Discover.' : '拖动模块调整顺序，并设置每个内容项在发现页的展示方式。'}
          </AppText>
        </View>
      </View>
      ...
      <Animated.View
        ...
        <View style={styles.editorRowHeader}>
          <View style={styles.dragHandle}>
            <AppIcon color={palette.textDim} name="moreDots" size={18} />
          </View>
          <View style={styles.flex}>
          ...
          <View style={styles.moveButtons}>
            <NativePressable
              ...
              <AppIcon color={palette.textDim} name="expandDown" size={13} style={styles.moveUpIcon} />
            </NativePressable>
            <NativePressable
