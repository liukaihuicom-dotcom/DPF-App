...
import { PartnerClientCard } from "@/src/components/business";
...
import { getFundingOperationActions, type FundingOperationAction } from "@/src/domain/funding";
    ...
    <Screen contentPadding="flush">
      <Card compact style={styles.accountPanel}>
        <View style={styles.accountPanelHeader}>
          ...
          >
            <AppIcon name="icon.market.watchlist" size={20} tone="tertiary" />
          </NativePressable>
        </View>
...
function AccountMenuSheet({
  ...
  const fundingActions = getSheetFundingActions(getFundingOperationActions(t, account.id), onClose);
  ...
  return (
    <View style={styles.accountMenuSheet}>
      ...
      <View style={StyleSheet.flatten([styles.menuListInset, { backgroundColor: colors.surface.panel }])}>
...
function getSheetFundingActions(actions: FundingOperationAction[], onClose: () => void) {
...
export function PartnerClientsScreen() {
  ...
  const openProfile = (clientId: string) => {
    toast.show({
      message: t("partner.clientProfileOpening"),
      title: t("upgrade.viewProfile"),
      tone: "default",
    });
  ...
  return (
    <Screen
      back
      subtitle={t("portfolio.partnerSubtitle")}
      title={t("portfolio.partnerTitle")}>
      <Card highlight style={styles.partnerSummaryCard}>
        <View style={styles.partnerSummaryHeader}>
          <View style={styles.partnerSummaryCopy}>
            <AppText variant="subtitle">{t("partner.clientBookTitle")}</AppText>
            <AppText tone="muted" variant="caption">
              {t("partner.clientBookHint")}
            </AppText>
          </View>
          <StatusPill compact label={t("partner.demoData")} tone="neutral" variant="outline" />
        </View>
        <View style={styles.metricRow}>
          ...
          <Metric label={t("upgrade.pendingCount")} tone={pending > 0 ? "amber" : "muted"} value={`${pending}`} />
        </View>
      </Card>

      {upgradeRequest.status === "pending" ? (
        <Card compact style={StyleSheet.flatten([styles.partnerPendingCard, { borderColor: colors.status.warning.fg }])}>
          <View style={styles.partnerPendingCopy}>
            <AppText variant="subtitle">{t("upgrade.pendingList")}</AppText>
            <AppText tone="muted" variant="caption">
              {t("partner.pendingReviewHint", { name: upgradeRequest.applicantName })}
            </AppText>
          </View>
          <StatusPill compact label={t("upgrade.status.pending")} tone="warning" />
        </Card>
      ) : null}

      <View style={styles.partnerListHeader}>
        <View style={styles.partnerSummaryCopy}>
          <AppText variant="title.section">{t("partner.clientListTitle")}</AppText>
          <AppText tone="muted" variant="caption">
            {t("partner.clientListHint")}
          </AppText>
        </View>
        <StatusPill compact label={t("partner.clientsCount", { count: partnerClients.length })} tone="info" />
      </View>

      {partnerClients.length > 0 ? (
        partnerClients.map((client) => <PartnerClientCard client={client} key={client.id} onOpenProfile={() => openProfile(client.id)} />)
      ) : (
        <EmptyState body={t("partner.emptyClients")} icon="icon.ib.network" title={t("partner.clientListTitle")} variant="card" />
      )}

      <AppText tone="dim" variant="caption">
        {t("partner.clientListRiskHint")}
      </AppText>
    </Screen>
  );
}
