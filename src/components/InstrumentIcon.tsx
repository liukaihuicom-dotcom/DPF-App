...
export function InstrumentIcon({ instrument, size = 36, style, symbol }: InstrumentIconProps) {
  ...
  if (kind === 'forex') {
    ...
    return (
      <View accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={StyleSheet.flatten([styles.root, { height: size, width: size }, style])}>
        <CurrencyMedallion currency={base} size={flagSize} style={styles.baseFlag} />
        <CurrencyMedallion currency={quote} size={flagSize} style={styles.quoteFlag} />
      </View>
    );
  }
  ...
  return (
    <AppIconFrame
      ...
      style={style}
      tone={assetTone}
      iconSize={Math.max(20, Math.round(size * 0.55))}
    />
  );
}
