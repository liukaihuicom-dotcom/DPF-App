...
type BottomSheetPresetBaseOptions = Pick<BottomSheetOptions, 'content' | 'footer' | 'snapPoints'>;
...
export function BottomSheetProvider({ children }: PropsWithChildren) {
  ...
  const hide = useCallback(() => setStack([]), []);
  ...
  return (
    <>
      ...
      <BottomSheetModal
        ...
        enableDynamicSizing={!options.snapPoints?.length}
...
function BottomSheetHeader({
  ...
  const nestedBackAction: BottomSheetHeaderAction = useMemo(
    () => ({
      accessibilityLabel: locale === 'en-US' ? 'Back' : '返回',
      icon: 'navigateBack',
      onPress: back,
    }),
    [back, locale],
      ...
      <HeaderIconSlot>
        {leftAction ? (
          ...
          <View
            ...
            <AppIcon color={palette.text} name={header.leftIcon} size={layout.headerIconSize} />
          </View>
        ) : null}
      </HeaderIconSlot>
