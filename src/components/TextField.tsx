...
import { AppIconFrame, type AppIconFrameBackgroundTone } from './AppIconFrame';
...
export function TextField({
  ...
  const iconNode =
    typeof icon === 'string' ? (
      <FieldLeadingIcon backgroundTone={getFieldStateIconBackgroundTone(resolvedState)} icon={icon as AppIconName} tone={iconTone} />
    ) : icon ?? null;
...
function FieldLeadingIcon({
  ...
  return <AppIconFrame backgroundTone={backgroundTone} name={icon} sizeVariant="md" iconSizeVariant="md" tone={tone} />;
}
...
function getFieldStateIconBackgroundTone(state: FormFieldState): AppIconFrameBackgroundTone {
  if (state === 'disabled') {
    return 'disabled';
  }
  ...
  if (state === 'focused' || state === 'inputting') {
    return 'brand';
  }
