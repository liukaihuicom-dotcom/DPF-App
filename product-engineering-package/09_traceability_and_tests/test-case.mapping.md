...
| `TC-AUTH-PIN-001` | Signed-in user opens a protected product route with default PIN state. | User is not redirected to PIN unlock; route opens directly because PIN is optional by default. | Major |
| `TC-AUTH-PIN-002` | User completes registration or password login without explicitly enabling PIN. | User enters the target route directly; PIN setup/unlock is not forced. | Major |
