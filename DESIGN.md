...
description: An institutional-grade crypto exchange whose marketing surfaces read like a quietly-confident financial-services brand. The base canvas is pure white; the single brand voltage is mapped to Dupoin Teal (`#2EB5C4`) in this project, used scarcely on signature glyphs, selected states, progress, and inline accent moments. Type runs Coinbase's licensed CoinbaseDisplay (display) and CoinbaseSans (body) at modest weights — display sits at weight 400 not 700, signaling editorial calm rather than fintech-bombastic. Page rhythm rotates between bright white sections, soft gray elevation bands, and full-bleed dark editorial heroes (`#0a0b0d`) carrying product-ui mockup cards. Iconography is geometric and minimal; depth comes from card-on-card layering, never decorative shadows.
...
Coinbase reads like an institutional financial brand that happens to trade crypto — the marketing surfaces are quiet, white-canvas, editorially-spaced, and almost monochromatic. The single brand voltage is mapped in this project to **Dupoin Teal** (`{colors.primary}` — #2EB5C4), used scarcely: selected states, focus, signature glyphs, progress, and inline emphasis links. Primary action buttons in this app deliberately stay neutral black/white/gray to preserve the existing broker product convention. Beyond that one teal, the system is white canvas + ink + soft gray elevation bands + a deep near-black editorial canvas (`{colors.surface-dark}` — #0a0b0d) for full-bleed product-mockup heroes.
...
**Key Characteristics:**
- Single accent color: `{colors.primary}` (#2EB5C4 Dupoin Teal) carries selected states, focus, progress, signature glyphs, and inline brand links. Used scarcely.
...
### Brand & Accent
- **Dupoin Teal** (`{colors.primary}` — #2EB5C4): The single brand accent for this implementation. Use it on selected states, focus, progress, signature glyphs, and inline brand links.
- **Dupoin Teal Active** (`{colors.primary-active}` — #2399A5): Press/focus accent darken where a teal accent is already present.
- **Dupoin Teal Disabled** (`{colors.primary-disabled}` — #B9E5EA): Faded teal tint for inactive accent marks.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on accent surfaces where teal is used.
...
**`button-primary`** — In the source inspiration this is a blue pill. In this app, primary buttons stay neutral black/white/gray; use `{colors.primary}` only for selected states, focus rings, progress, and small brand accents.

**`button-primary-active`** — Press state for teal accents only. Primary buttons should use the neutral button system.

**`button-primary-disabled`** — Faded teal tint for disabled accent moments. Disabled buttons should use neutral gray.
...
**`button-pill-cta`** — Larger pill CTA used on the homepage hero ("Get started"). In this app it uses the neutral button system with 56px height and 16px × 32px padding for a prouder stance.
...
**`text-input`** — Standard text input. Background `{colors.canvas}`, text `{colors.ink}`, rounded `{rounded.md}` (12px), padding 14px × 16px, height 48px, 1px hairline border. On focus, border may thicken to 2px Dupoin Teal.
...
### Do
- Reserve `{colors.primary}` (Dupoin Teal) for selected states, focus, progress, brand-glyph illustrations, and inline accent links.
- Keep primary buttons neutral black/white/gray in this app.
...
### Don't
- Don't introduce a secondary brand color. Dupoin Teal is the only brand accent; trading green/red are semantic-only.
...
7. Dupoin Teal stays scarce — one or two teal moments per band.
