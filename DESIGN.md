...
description: A warm, generous mobile marketplace style inspired by Airbnb, anchored on a clean white canvas and mapped to Dupoin Teal (#2EB5C4) as the single brand accent. Type runs Airbnb Cereal VF at modest weights — display sits at 22–28px in weight 500/600 rather than the heavy 700+ that fintech and enterprise systems use; the design trusts generous whitespace, friendly rounded surfaces, and clear entry points over visual noise. Pill-shaped search bars (`{rounded.full}`), softly rounded cards (`{rounded.lg}` ~14px), and 32px button radii read as friendly and human — adapted here for a mobile FX/CFD broker app.

projectOverrides:
  brandMapping: "Airbnb is a style reference only. In this project, every primary/accent use maps to Dupoin Teal #2EB5C4, never Airbnb red."
  buttonMapping: "Primary trading and product CTAs keep the existing neutral black/white/gray button system. Use #2EB5C4 only for selected states, focus, progress, badges, icons, and small accent moments."
  productContext: "Apply the warm spacing, rounded search surfaces, friendly cards, and human marketplace clarity to a mobile FX/CFD broker app without copying Airbnb assets, logos, copy, or brand color."

colors:
  primary: "#2EB5C4"
  primary-active: "#2399A5"
  primary-disabled: "#B9E5EA"
...
**Key Characteristics:**
- Single accent color: `{colors.primary}` (#2EB5C4 — "Dupoin Teal") carries every primary CTA, the search orb, the heart save state, and the brand wordmark. Used scarcely — most pages are 90% white + ink with one or two Dupoin Teal moments.
...
- Pill-shaped global search bar: white surface, fully rounded (`{rounded.full}`), divided by 1px hairlines into Where / When / Who segments, terminated by a circular Dupoin Teal search orb (`{component.search-orb}`).
...
### Brand & Accent
- **Dupoin Teal** (`{colors.primary}` — #2EB5C4): The single brand color. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on property cards, and inline brand links. The most recognizable color in consumer travel.
- **Dupoin Teal Active** (`{colors.primary-active}` — #2399A5): The press / pointer-down variant — slightly more saturated. Used on `{component.button-primary-active}`.
- **Dupoin Teal Disabled** (`{colors.primary-disabled}` — #B9E5EA): A pale tint used on disabled CTAs.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Dupoin Teal CTAs.

### Semantic
- **Error** (`{colors.primary-error-text}` — #c13515): Inline error text for form validation. Distinct from Dupoin Teal — slightly darker, more saturated red.
...
**`button-primary`** — Dupoin Teal fill, white text, 8px radius, 14×24px padding, 48px height, weight 500. The most common CTA across the system: "Reserve", "Continue", "Search", account-flow primaries.

**`button-primary-active`** — The press state. Background flips to `{colors.primary-active}`. No transform, no shadow change.

**`button-primary-disabled`** — Pale Dupoin Teal tint at #B9E5EA with white text. Cursor not-allowed.

**`button-secondary`** — White fill with ink text and a 1px ink outline. 8px radius. Used for "Save", "Cancel", and inverse CTAs over Dupoin Teal surfaces.
...
**`button-pill-rausch`** — A pill-shaped Dupoin Teal CTA used on featured cells (e.g., "Become a host" sub-CTA) — 9999px radius, 10×20px padding, 14px label.
...
**`search-orb`** — The circular Dupoin Teal orb terminating the right edge of the search bar. 48×48px, fully rounded, white magnifying-glass icon centered. The hottest single color moment on the homepage.
...
**`property-card`** — A photo-first card. 1:1 aspect-ratio image with `{rounded.md}` corner clipping, image carousel dots overlay, "Guest favorite" floating badge top-left (`{component.guest-favorite-badge}`), and a heart icon top-right (`{component.icon-button-circle}` in default outlined state, Dupoin Teal-filled when saved). Beneath the image: 4–5 lines of meta — title (`{typography.title-md}`), distance / dates (`{typography.body-sm}` muted), and price ("$X night") right-aligned.
...
- **Map view styling:** the search-results map uses Mapbox-tinted tiles with custom Dupoin Teal markers; not captured here.
