...
description: "Use for production UI page builds, HTML, React, Vue, state matrices, interaction feedback, visual QA, responsive QA, accessibility QA, and developer handoff from Page Contract and Design System inputs."
---
...
## Add-on: UI Reference Adaptation Add-on Skill

| 项目 | 声明 |
|---|---|
| Add-on Name | UI Reference Adaptation Add-on Skill |
| Path | `skills/ui-build-production/addons/reference-adaptation/SKILL.md` |
| Purpose | 当任务包含 Mobbin、App 截图、Web Reference、Dribbble、Behance、Figma Community、竞品截图或“参考这个页面风格”时，先将参考案例转化为 Visual DNA、Token Mapping、Component Mapping、Pattern Mapping、Business Adaptation，再交给 UI Build 执行。 |
| Rule | Add-on 只输出 UI Build Reference Input，不直接生成最终页面，不替代 Page Contract，不绕过 Design System 和 Component Manifest。 |
