...
|---|---|---|---|---|---|---|---|
| Home Markets | `/` | Markets & Instruments | `app/(tabs)/index.tsx` | trader and partner see market context | Dupoin hero, quote status, account strip, top movers, copy signal preview, favorites, market brief | `BrokerStore`, `dupoinMvp` | No live CMS or production instrument service |
...
| Login | `/auth` | Auth & Onboarding | `app/auth/index.tsx` | guest sign-in page | Email/password validation, local sign-in, social unavailable toast, register/reset links | form state, `setAuthStatus` | No real identity provider |
| Register | `/auth/register` | Auth & Onboarding | `app/auth/register.tsx` | guest account creation page | Email/password/confirm validation, invite code, risk checkbox, local sign-in | form state, `setAuthStatus` | No account creation API or risk record |
...
|---|---|---|---|
| `/markets` | `app/(tabs)/markets.tsx` | redirects to `/` | Compatibility alias for market home |
