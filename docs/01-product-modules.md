...
|---|---|---|---|---|
| Auth & Onboarding | Local sign-in, registration, password reset, first-week activation, risk acknowledgement | `/auth`, `/auth/register`, `/auth/forgot-password`, `/auth/onboarding` | guest, trader, partner | `ProductSettingsProvider`, local form state |
...
| Account Assets | Demo balance, equity, margin, account profiles, account detail analytics, transaction preview | `/accounts`, `/account`, `/account-details/[id]` | trader, partner | `initialAccount`, `buildTradingAccountProfiles` |
| Discover & Growth | Function center, selected module status, copy signal preview, academy/challenge/support/reward placeholders | `/discover`, `/quick` | trader, partner | `dupoinMvp`, selected discover module settings |
...
- Target users: trader, partner.
- Entry routes: `/accounts`, `/account`, `/account-details/[id]`.
...
- Target users: trader, partner.
- Entry routes: `/discover`, `/quick`.
- Implemented functions: function entry grid, selected module status, copy signal cards, follow placeholder, Dupoin Academy card, demo challenge card, support desk card, risk controls card, Partner preview stats, market brief cards, persistent selected module per role.
- Core actions: select discover module, jump to markets/order/accounts/onboarding/partner tools, preview unavailable modules through toast.
- State source: `dupoinMvp` static content and `selectedDiscoverModuleByRole`.
